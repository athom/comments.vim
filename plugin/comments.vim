...
function! CommentLine()
  ...
  " for .h .c, .cpp or .hpp or .java or .js or arduino (*.ino or *.pde) files use //
  if file_name =~ '\.c$' || file_name =~ '\.h$' || file_name =~ '\.cpp$' || file_name =~ '\.hpp$' || file_name =~ '\.java$' || file_name =~ '\.php[2345]\?$' || file_name =~ '\.C$' || file_name =~ '\.ino$' || file_name =~ '\.pde$' || file_name =~ '\.go$' || file_name =~ '\.js$'
    execute ":silent! normal ^i//\<ESC>\<down>^"
  " for .c or .h or .pc or .css files use /* */
  elseif file_name =~ '\.pc$' || file_name =~ '\.css$'
